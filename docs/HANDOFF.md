...
5. `fly-deploy.yml`의 `paths` 필터에 **`requirements.txt` 추가** — 현재 필터는
   `**.py`/`data/**`/`Dockerfile`/`fly.toml`뿐이라 **의존성만 바꾼 커밋은 재배포되지
   않는다**. 위 2번(2.0.0 업그레이드)을 실행할 때 핀만 올리면 원격에 반영이 안 되므로
   그 전에 처리 필요 (지금은 무해 — 주간 data 커밋이 곧 재빌드를 유발하므로 핀은
   다음 주간 배포에서 자동 적용된다)
