...
### ⚠️ 실질 리스크 1건: SDK 버전 미고정 드리프트
`requirements.txt`가 `mcp` **버전 미고정** → 주간 자동 배포가 Docker 이미지를 다시 빌드할 때마다 최신 SDK가 설치됨. Python Tier 1 SDK는 이미 2026-07-28을 지원하므로, **어느 주간 배포에서 조용히 신형 SDK로 넘어갈 수 있고**, 구버전 클라이언트(claude.ai 커넥터 등)와의 호환은 SDK의 다중 버전 협상에 달려 있다 (설치본은 mcp 1.28.1 — 신스펙 지원 여부 미확인).

### v2.2 권장 작업 (착수 시 기존 사이클: 브레인스토밍→스펙→계획→SDD)
1. **SDK 핀 고정** (`mcp==1.28.1`) — 드리프트 즉시 차단 (한 줄, 최우선)
2. 최신 SDK로 **의도적 업그레이드 검증**: 128 테스트 + 로컬/원격 remote_smoke + claude.ai 커넥터(구버전 클라이언트 호환) 실연결 확인 후 핀 갱신
