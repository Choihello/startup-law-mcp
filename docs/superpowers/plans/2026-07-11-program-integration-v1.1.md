...
**필드명은 2026-07-11 실 probe로 확정된 값이다** (data/_cache/probe_programs_*.json 참조). 사업소개는 기관·연락처·지역·접수기간 필드가 없으며, 지원내용·특징·예산 필드는 summary에 합쳐 보존한다.
...
Expected: 8 passed (신규 4 + 기존 유틸 4). 전체 62 passed.
...
### Task 3: 수집(현행 공고 필터·조기중단) + 원자적 스냅샷 (`sync`)

**probe 확정 사실 (2026-07-11):** 공고 API는 전체 아카이브 **29,430건**을 반환하며 서버측 필터(cond, plain param 모두)는 무시된다. 다만 기본 정렬이 모집중(`rcrt_prgs_yn=="Y"`) 우선이라 Y 밀도가 페이지에 따라 급감한다(1p 92% → 10p 3% → 30p 0%). 따라서 공고는 **클라이언트 필터 + 조기중단** 전략으로 현행분만 수집한다. 사업소개는 511건 전량 수집.
...
**Interfaces:**
- Consumes: `fetch_page`, `_date_norm`, `normalize_announcement`, `normalize_intro`, `PROGRAMS_DIR`(호출 시점 전역 참조 — 테스트가 monkeypatch)
- Produces:
  - `program_sync.fetch_all(key, target, per_page=100, max_pages=50) -> list[dict]` — 전량 수집 (사업소개용)
  - `program_sync.fetch_current_announcements(key, per_page=100, max_pages=50, today=None) -> list[dict]` — 현행 공고만 (Y 플래그 또는 마감 미래), 0건 페이지 2연속이면 중단
  - `program_sync.sync(key: str) -> dict` — 반환 `{"announcements": int, "intros": int, "fetched_at": ISO}`. 스냅샷 파일 구조 `{"fetched_at": ISO, "count": n, "items": [...]}`. **전체 수집 성공 후에만 파일 쓰기.** Task 5 `sync_programs` 도구가 호출.

- [ ] **Step 1: 실패하는 테스트 작성**

`tests/test_program_sync.py` (필드명은 probe 확정값):
```python
import json
...
Expected: 66 passed (Task 1 fix 포함 62 + 신규 4)

- [ ] **Step 5: Commit**

```bash
git add program_sync.py tests/test_program_sync.py
git commit -m "feat: 현행 공고 필터 수집(조기중단) + 원자적 스냅샷 sync"
```
...
Run: `python -m pytest tests/ -q`
Expected: 73 passed (test_server.py 2개는 교체이므로 총계 유지)
...
Run: `python -m pytest tests/ -q`
Expected: 73 passed. `git grep`으로 인증키 유출 없음 확인 (`git grep -i data_go_kr_key`는 env 참조 코드만 나와야 함, 키 값 자체는 0건).
