...
원격판은 관리 도구(`sync_programs`)를 제외한 **13개 도구**만 노출한다(데이터 갱신은 자동
재배포로 처리). 유휴 시 머신이 절전 상태로 내려가므로 첫 요청은 콜드스타트로 수 초 지연될
수 있다 — 이후 요청은 빠르다. 로컬에서 전체 14개 도구(관리 포함)로 직접 실행하려면
아래 "빠른 시작"을 따른다.
...
Claude Desktop을 완전히 재시작하면 도구 14개가 인식된다.
...
`server.py`(stdio, 로컬용 14개 도구)와 `server_http.py`(streamable-http, 원격 배포용
13개 도구·`sync_programs` 제외)는 같은 `register_tools()`를 공유하며 `data/`를 그대로
읽는다.
...
- **v2.0 원격 배포** — `server_http.py`(streamable-http, 12개 도구) + Docker화 +
