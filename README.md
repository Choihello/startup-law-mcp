...
Claude Desktop을 완전히 재시작하면 도구 10개가 인식된다.
...
> 도구에서 1~50 범위의 정수만 허용합니다(벗어나면 `invalid_input` 반환).
...
v1.0/v1.1 모두 수동 실행이 원칙이다(로드맵 v1.4에서 자동화 예정).
...
- **v1.1** — K-Startup 지원사업 통합: `search_program`/`get_program`/
  ...
  511건 초기 동기화
- **v1.2 안정화 패치 (현재)** — 동기화 방어 로직(0건·급감 70%·스키마 검증·원자 교체),
...
- **v1.3** — 창업 특화 3도구: `delegation_map`(법률→시행령·시행규칙 위임 조문 자동
  연결), `startup_stage_guide`(창업 단계별 관련 조문 큐레이션), `check_effective_date`
  (조문 시행일·경과규정 확인)
- **v1.4** — GitHub Actions 주간 동기화(법령+지원사업) → 변경 감지 시 자동 PR
